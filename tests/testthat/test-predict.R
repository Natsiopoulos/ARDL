test_that("verify prediction results all cases", {
    ...
    case_i <- data.frame(predictions = c(-13.1721023693382, -40.3134240474061, -57.8052029786566))
    case_ii_iii <- data.frame(predictions = c(-7.40819925882942, -24.8395014334613, -29.1887358923117))
    case_iv_v <- data.frame(predictions = c(-6.61455842285156, -23.5381169607463, -27.919945003499))
...
test_that("verify equivalence of different newdata types", {
    ...
    expect_equal(c(result_df[,"predictions"]), c(unname(result_ts_timestamp)))
    # zoo object
    ...
    expect_equal(c(result_df[,"predictions"]), c(coredata(result_zoo_timestamp)))
    # zooreg object
    ...
    expect_equal(c(result_df[,"predictions"]), c(coredata(result_zooreg_timestamp)))
})
